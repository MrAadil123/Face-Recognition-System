...
class App:
    ...
    def login(self):
        ...
        rgb_img = cv2.cvtColor(self.most_recent_capture_arr, cv2.COLOR_BGR2RGB)
        cv2.imwrite(unknown_img_path, rgb_img)
        ...
        if lines:
    # lines[0] is like './.tmp.jpg,Aadil'
            parts = lines[0].split(',')
            if len(parts) >= 2:
                name = self.clean_username(parts[1])
    ...
    def accept_register_new_user(self):
        raw_name = self.entry_text_register_new_user.get(1.0, "end-1c")
        name = self.clean_username(raw_name)
